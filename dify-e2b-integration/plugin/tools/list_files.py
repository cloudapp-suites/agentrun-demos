"""List files in a directory in a self-hosted E2B sandbox."""
...
class ListFilesTool(Tool):
    ...
    def _do_invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            files = [{"name": e.name, "type": e.type, "path": e.path} for e in entries]
            yield self.create_text_message(f"Found {len(files)} files in {path}")
